...
class PromptTemplateService
{
    ...
    public function saveTemplateWithFields(PromptTemplate $model, array $postData, array $fieldsMapping): bool
    {
        ...
        if (!empty($invalidPlaceholders)) {
            return $this->addValidationErrorAndLoad(
                $model,
                $postData,
                'Invalid field placeholders found: ' . implode(', ', $invalidPlaceholders)
            );
        }
        ...
        if (!empty($duplicatePlaceholders)) {
            return $this->addValidationErrorAndLoad(
                $model,
                $postData,
                'Duplicate field placeholders found: ' . implode(', ', $duplicatePlaceholders)
            );
        }
    ...
    {
        $content = $this->extractContentFromDelta($template);
    ...
    private function extractContentFromDelta(string $template): string
    {
    ...
    private function addValidationErrorAndLoad(PromptTemplate $model, array $postData, string $errorMessage): bool
    {
    ...
    {
        return $this->getTemplatesContentByUser($userId, $projectId);
    }

    public function getTemplatesContentByUser(int $userId, ?int $projectId = null): array
    {
        $query = PromptTemplate::find()
            ->joinWith('project')
            ->where(['project.user_id' => $userId]);
    ...
    public function convertPlaceholdersToLabels(string $template, array $fieldsMapping): string
    {
        $delta = json_decode($template, true);
        if (!$delta || !isset($delta['ops'])) {
            return $template;
        ...
        $normalizedMapping = $this->normalizeFieldsMappingForLabels($fieldsMapping);
    ...
    private function normalizeFieldsMappingForLabels(array $fieldsMapping): array
    {
        $normalized = [];
        foreach ($fieldsMapping as $placeholder => $data) {
            if (preg_match('/^(GEN|PRJ|EXT):\{\{(.+)}}$/', $placeholder, $matches)) {
                ...
                $fieldName = $matches[2];
                $fieldId = $data['id'];
                $normalized[$prefix][$fieldId] = $fieldName;
            }
        }
        return $normalized;
    }
