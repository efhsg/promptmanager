...
De applicatie moet draaien vanuit `/var/www/worktree/main/` in plaats van `/var/www/html/`.
Dit introduceert een directory-niveau (`worktree/`) zodat toekomstige worktrees als siblings
naast `main/` kunnen bestaan (bijv. `bugfix-123/`, `feat-x/`).
...
Door het pad te verschuiven naar `/var/www/worktree/main`:
...
- Nieuwe env var `APP_ROOT` met default `/var/www/worktree/main`
...
/var/www/worktree/
├── main/                       <- main
├── bugfix-123/                 <- worktree (fase 2)
└── feat-x/                     <- worktree (fase 3/4)
```
...
| 4 | `linter.sh` | 1 | Config pad → `${APP_ROOT}`. NB: draait op host via `docker exec` — gebruik `${APP_ROOT:-/var/www/worktree/main}` fallback |
| 5 | `linter-staged.sh` | 2 | Padconversie + config → `${APP_ROOT}`. NB: draait op host via `docker exec` — gebruik `${APP_ROOT:-/var/www/worktree/main}` fallback |
| 6 | `.vscode/launch.json` | 1 | Xdebug path mapping → `/var/www/worktree/main` |
| 7 | `docker/yii/codex-config.toml` | 1 | Project sectie → `/var/www/worktree/main` |
| 8 | `.env.example` | 0 | Nieuwe var `APP_ROOT=/var/www/worktree/main` toevoegen |
...
2. **Single source of truth:** `APP_ROOT` is de enige plek waar het pad gedefinieerd wordt.
   ...
   `codex-config.toml` gebruiken het hardcoded pad `/var/www/worktree/main` omdat die
   formaten geen env var substitutie ondersteunen.
3. **Backward compatible default:** `APP_ROOT` default is `/var/www/worktree/main`. Als de
...
2. Als `APP_ROOT` ontbreekt of leeg is: fallback `/var/www/worktree/main`

### Propagatieketen

1. `docker-compose.yml` levert `APP_ROOT` aan `pma_yii`, `pma_queue` en `pma_nginx`
2. `docker-compose.yml` gebruikt `${APP_ROOT:-/var/www/worktree/main}` voor mounts, `working_dir` en PATH-gerelateerde waarden
3. `docker/yii/Dockerfile` gebruikt `ARG APP_ROOT=/var/www/worktree/main` voor build-time padoperaties
...
- Host scripts (`linter.sh`, `linter-staged.sh`): gebruiken `${APP_ROOT:-/var/www/worktree/main}` fallback bij `docker exec`
...
4. **Empty/NVT:** als `APP_ROOT` niet is gezet, melden scripts expliciet dat fallback `/var/www/worktree/main` wordt gebruikt.
...
| `.env` niet bijgewerkt door gebruiker | APP_ROOT is leeg, defaults vallen terug | Fallback `${APP_ROOT:-/var/www/worktree/main}` overal |
...
| T1.2 | `docker exec pma_yii pwd` | Toont `/var/www/worktree/main/yii` |
| T1.3 | `docker exec pma_nginx cat /etc/nginx/nginx.conf \| grep root` | Toont `/var/www/worktree/main/yii/web` |
...
| T1.8 | `docker exec pma_yii php -i \| grep error_log` | Toont `/var/www/worktree/main/yii/runtime/logs/php_errors.log` |
| T1.9 | `docker exec pma_yii ls /var/www/worktree/main/yii/web/index.php` | Bestand bestaat — volume mount werkt |
| T1.10 | APP_ROOT env var niet gezet (default) | App gebruikt default `/var/www/worktree/main`. Test door T1.1-T1.9 uit te voeren zonder `APP_ROOT` in `.env`. Fallback via `:-` in docker-compose en `ARG` default in Dockerfile. |
| T1.11 | `docker compose exec -T pma_queue pwd` | Toont `/var/www/worktree/main/yii` — `pma_queue` paden zijn ook gemigreerd |
| T1.12a | `APP_ROOT=../tmp docker compose config` | Compose-config check faalt of geeft duidelijk signaal dat waarde ongeldig is |
| T1.12b | `APP_ROOT='/var/www/worktree/main;rm -rf /' ./linter.sh check` | Faalt direct met duidelijke foutmelding over ongeldige `APP_ROOT` waarde |
| T1.13 | `APP_ROOT` unset en `./linter.sh check` uitvoeren | Output meldt expliciet dat fallback `/var/www/worktree/main` gebruikt wordt |
| T1.14 | Ongeldige `APP_ROOT` met `./linter.sh check` | Foutmelding gebruikt vast format met oorzaak + herstelhint, en exitcode is `2` |
| T1.15 | Preconditie: `touch yii/models/_qa_app_root_probe.php && git add yii/models/_qa_app_root_probe.php`. Daarna `APP_ROOT` unset en `./linter-staged.sh` uitvoeren | Output meldt expliciet dat fallback `/var/www/worktree/main` gebruikt wordt. Na test: `git restore --staged yii/models/_qa_app_root_probe.php && rm yii/models/_qa_app_root_probe.php` |
